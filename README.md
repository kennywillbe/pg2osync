...
Elasticsearch, Meilisearch, a pgvector table or Qdrant within milliseconds.
...
```sh
v=v1.5.2 t=x86_64-unknown-linux-musl   # or aarch64-unknown-linux-musl,
...
<!-- x-release-please-start-version -->
Or run the image: `docker run ghcr.io/kennywillbe/pg2osync:1.5.2`.
<!-- x-release-please-end -->
...
| Vectors on a Qdrant target | ✅ the collection's named vectors are the mapping: a document field of the same name becomes that vector and everything else becomes payload; a document id becomes a deterministic UUID point and is kept in the `_pg2osync_id` payload field; the sink computes nothing |
...
**Target** — OpenSearch 2.x, Elasticsearch 8.x, Meilisearch 1.x, PostgreSQL
with pgvector, or Qdrant 1.x as a search backend.
...
docker run --rm \
  ...
  -p 9100:9100 \
  ghcr.io/kennywillbe/pg2osync:1.5.2
```
<!-- x-release-please-end -->
...
- Sinks: [OpenSearch](docs/sinks/opensearch.md) · [Elasticsearch](docs/sinks/elasticsearch.md) · [Meilisearch](docs/sinks/meilisearch.md) · [PostgreSQL (pgvector)](docs/sinks/postgresql.md) · [Qdrant](docs/sinks/qdrant.md)
...
├── sink/          OpenSearch / Elasticsearch / Meilisearch / PostgreSQL / Qdrant writers
