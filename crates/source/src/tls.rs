//! TLS for every PostgreSQL connection this tool opens.
//!
//! Modes mirror libpq's `sslmode` so a connection string copied from a managed
//! provider behaves the way its documentation says it will. One module owns
//! both connection kinds — ordinary SQL connections and the replication
//! transport — so they can never drift into different security postures.
...
impl TlsSettings {
    ...
    pub async fn connect(&self, url: &str) -> Result<Client> {
        ...
        // certificate and hostname checking is the connector's job below.
        config.ssl_mode(if self.mode.requires_tls() {
            tokio_postgres::config::SslMode::Require
        } else {
            tokio_postgres::config::SslMode::Prefer
        });

        let connector = tokio_postgres_rustls::MakeRustlsConnect::new(self.client_config()?);
        let (client, connection) = config
            .connect(connector)
            .await
            .with_context(|| format!("connection failed (sslmode={})", self.mode.as_str()))?;
        tokio::spawn(async move {
            if let Err(e) = connection.await {
                tracing::debug!(target: "pg2osync::tls", "connection closed: {e}");
            }
    ...
    pub fn replication_config(&self) -> pgwire_replication::TlsConfig {
        ...
        pgwire_replication::TlsConfig {
            ...
            ca_pem_path: self.root_cert.clone(),
            sni_hostname: None,
...
mod tests {
    ...
    fn replication_config_mirrors_the_mode() {
        let settings = TlsSettings::resolve("postgres://u:p@h/db", Some("require"), None).unwrap();
        assert_eq!(
            settings.replication_config().mode,
